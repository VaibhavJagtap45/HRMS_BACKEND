...
async function updateMe(req, res) {
  try {
    ...
    if (bankName !== undefined) user.bankName = String(bankName).trim();
    if (bankAccountNo !== undefined)
      user.bankAccountNo = String(bankAccountNo).trim();
    if (ifscCode !== undefined)
      user.ifscCode = String(ifscCode).trim().toUpperCase();
    if (branchName !== undefined) user.branchName = String(branchName).trim();
...
async function forgotPassword(req, res) {
  try {
    ...
    if (!identifier || !String(identifier).trim()) {
      return res.status(400).json({
        message: "Please provide your email or employee ID.",
      });
    }
    ...
    if (!emailSent) {
      return res.json({
        ...
        _dev_resetUrl: resetUrl,
        _dev_note:
          "SMTP not configured. Use this URL to reset the password (dev only).",
      });
    }
  ...
  try {
    ...
    if (!sent) {
      return res.json({
        ...safeRes,
        _dev_otp: otp,
        _dev_note: "SMTP not configured. OTP returned for dev only.",
      });
    }
  ...
  try {
    ...
    const user = await User.findOne({
      $or: [
        ...
        { employeeId: String(identifier).trim().toUpperCase() },
      ],
    }).select(
      "+passwordResetOtp +passwordResetOtpExpires +passwordResetVerifiedToken +passwordResetVerifiedExpires",
    );
    ...
    return res.json({
      ...
      verifiedToken: rawVerifiedToken,
    });
  } catch (error) {
...
async function resetPasswordOtp(req, res) {
  try {
    ...
    if (!verifiedToken || !password) {
      return res.status(400).json({
        message: "Verified token and new password are required.",
      });
    }
    ...
    const user = await User.findOne({
      ...
      "+passwordResetVerifiedToken +passwordResetVerifiedExpires +password",
    );
